...
export default class RouterGallery extends Component<Props> {
  ...
  render() {
    ...
    return (
      <div>
        <Heading source="/Patterns/RouterGallery.js">
          Carousel with Routing
        </Heading>
        <p>
          In this example we sync the <Code>currentIndex</Code> with the URL
          using react-router's <Code>history.push</Code> method. Check out the
          source to see this technique in detail.
        </p>
        <p>
          ...
          refresh via react-router's <Code>match.params</Code>.
        </p>
        {!isLoading ? (
          <Carousel
